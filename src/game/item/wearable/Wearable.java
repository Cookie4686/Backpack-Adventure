...
public abstract class Wearable extends Item implements TurnActivable, StatUpdatable, ReStatable {
	...
	private int shield, increaseShield;

	public Wearable(String name, String detail, int initialShield, int increaseShield, ArrayList<Effect> effects,
	...
	public void activatePerTurn() {
		for (Effect effect : effects) {
			if (effect.getType() != EffectType.LUCK && effect.getType() != EffectType.DODGE
					&& effect.getType() != EffectType.HEAL && effect.getType() != EffectType.ENERGY) {
	...
	protected String getProvide() {
		...
		if (shield > initialShield)
			text = text + " + " + (shield - initialShield);
		else if (shield < initialShield)
			text = text + " - " + (initialShield - shield);
